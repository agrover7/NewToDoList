...
function completeTodo(event) {
  ...
  completeRequest.onreadystatechange = function(complete) {
    if (this.readyState == 4 && this.status == 200) {
  ...
  if (event.target.parentNode.classList.contains("completed")) //we are changing it from completed to uncompleted
  {
    completeRequest.send(JSON.stringify(data_uncomplete));
  } else {
    completeRequest.send(JSON.stringify(data_complete));
  }
}
